function GOAP_Planner() constructor {
  ...
  read_state = function(_state, _key) {
    mark_key(_key);
    return variable_struct_exists(_state, _key) ? variable_struct_get(_state, _key) : undefined;
  };
  read_mem = function(_memory, _key, _default) {
    mark_key(_key);
    return _memory.read(_key, _default);
  };
  var _keys_scratch = [];
  var _open_best_g = ds_map_create();
  ...
  var _normalize_predicate = function(_entry) {
    var _result = { key: undefined, value: true, negate: false, op: "eq" };
    if (is_string(_entry)) {
    ...
    if (is_struct(_entry) && variable_struct_exists(_entry, "op")) {
      var _op_value = variable_struct_get(_entry, "op");
      if (!is_undefined(_op_value)) {
        _result.op = string_lower(string(_op_value));
      }
    }
    return _result;
  };
  ...
  var _default_check_preconditions = function(_action, _state) {
    ...
    for (var _i = 0; _i < _len; ++_i) {
      ...
      if (!_pred_ok(_predicate, _state)) return false;
    }
    return true;
  };
  ...
  var _goal_unsatisfied_count = function(_goal, _state) {
    ...
    for (var _i = 0; _i < _len; ++_i) {
      ...
      if (is_undefined(_predicate.key)) continue;
      _mark_referenced_key(_predicate.key);
      if (!_pred_ok(_predicate, _state)) _count += 1;
    }
    return _count;
  };
  ...
  _search_a_star = function(_initial_state, _goal, _actions, _beliefs, _memory) {
    ...
    var _root_hash = _state_hash(_root_state);
    ds_priority_add(_open, _root, _root.f);
    _open_count = 1;
    var _open_peak = 1;
    ds_map_add(_open_best_g, _root_hash, 0);
    ...
    while (!ds_priority_empty(_open)) {
      ...
      if (_had) {
        ...
        if (_current.g >= _best_g) { continue; }
      ...
      if (is_array(_actions)) {
        ...
        for (var _i = 0; _i < _len_actions; ++_i) {
          ...
          if (ds_map_exists(_closed, _hash_next)) {
            ...
            if (_g >= _best_closed && !config.reopen_closed_on_better_g) { _should_add = false; }
          }
          if (_should_add) {
            if (ds_map_exists(_open_best_g, _hash_next)) {
              var _best_open = ds_map_find_value(_open_best_g, _hash_next);
              if (_g >= _best_open) { _should_add = false; }
              else { ds_map_replace(_open_best_g, _hash_next, _g); }
            } else {
              ds_map_add(_open_best_g, _hash_next, _g);
            }
          }
          if (_should_add) {
  ...
  _state_hash = function(_state) {
    ...
    _keys_scratch = variable_struct_get_names(_state);
    var _keys = _keys_scratch;
    if (array_length(_keys) > 1) {
      array_sort(_keys, function(a, b) {
